...
ratings %>%
  ...
  labs(x="Rating (1 to 10)", y="Distribution of votes", 
       title = sprintf("All IMDB users (tot. votes = %s)", 
...
ratings %>%
  ...
  scale_x_discrete(breaks = c("1", rep("",8), "10")) +
  labs(x="Rating (1 to 10)", y="Distribution of votes", 
       title = sprintf("Female IMDB users (tot. votes = %s)", 
                       format(sum(ratings$Votes[
...
ratings %>%
  ...
  scale_x_discrete(breaks = c("1", rep("",8), "10")) +
  labs(x="Rating (1 to 10)", y="Distribution of votes", 
       title = sprintf("Over 45yo IMDB users (tot. votes = %s)", 
                       format(sum(ratings$Votes[
...
ratings %>%
  ...
  scale_x_discrete(breaks = c("1", rep("",8), "10")) +
  labs(x="Rating (1 to 10)", y="Distribution of votes", 
       title = sprintf("Females over 45yo IMDB users (tot. votes = %s)", 
                       format(sum(ratings$Votes[
...
ratings %>%
  ...
  labs(x="Episode", y="Rating", colour="Season") +
...
ratings %>%
  ...
  labs(x="Episode", y="Female users", colour="Season") +
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank())
